#!/usr/bin/env python3
"""
Minimal Enhancement: Baseline + Extended Lags Only.

The simplest enhancement: just add more lag features.
No rolling stats, no temporal, no eve data.
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Minimal enhancement: extended lags only")
...
def make_supervised_features(df: pd.DataFrame, lags: Sequence[int]) -> pd.DataFrame:
    """Same as baseline but with extended lags."""
    rows = []
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print("=" * 70)
    print("MINIMAL ENHANCEMENT: Extended Lags Only")
    print("=" * 70)
    print(f"\nLags: {args.lags}")
    print(f"Baseline lags were: [1, 2, 7, 14, 28]")
    print(f"New lags added: [3, 21, 56]")
    ...
    n = len(results)
    print(f"\n{'='*70}")
    print("POOLED COVERAGE RESULTS (Extended Lags Only)")
    print("="*70)
    print(f"\nHoldout: {n} days evaluated")
    print("\n| Top-K | Avg Cov | 100% Days | 75%+ Days | Baseline | Delta |")
    print("|-------|---------|-----------|-----------|----------|-------|")
    ...
    # Check new lags contribution
    new_lag_imp = sum(imp for f, imp in feat_imp if any(f"lag{l}" in f for l in [3, 21, 56]))
    old_lag_imp = sum(imp for f, imp in feat_imp if any(f"lag{l}" in f for l in [1, 2, 7, 14, 28]))
    print(f"\nNew lags (3,21,56) importance: {new_lag_imp:.4f}")
    print(f"Old lags (1,2,7,14,28) importance: {old_lag_imp:.4f}")

    output = {
        "experiment": "extended_lags_only",
        "lags": list(args.lags),
    ...
    out_path = os.path.join(args.out_dir, "holdout_lags_only_results.json")
