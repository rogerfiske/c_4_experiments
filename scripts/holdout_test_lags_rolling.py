#!/usr/bin/env python3
"""
Enhancement: Extended Lags + Rolling Means.

Building on the lags-only success, add rolling means (no std).
"""
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Extended lags + rolling means")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print("=" * 70)
    print("ENHANCEMENT: Extended Lags + Rolling Means")
    print("=" * 70)
    print(f"\nLags: {args.lags}")
    print(f"Rolling windows (mean only): {args.rolling_windows}")
    ...
    n = len(results)
    print(f"\n{'='*70}")
    print("POOLED COVERAGE RESULTS (Lags + Rolling Means)")
    print("="*70)
    print(f"\nHoldout: {n} days evaluated")
    print("\n| Top-K | This | Lags-Only | Baseline | vs Base |")
    print("|-------|------|-----------|----------|---------|")

    baselines = {1: 0.282, 2: 0.535, 3: 0.695, 5: 0.912, 7: 0.972}
    lags_only = {1: 0.310, 2: 0.532, 3: 0.715, 5: 0.908, 7: 0.982}

    for k in [1, 2, 3, 5, 7]:
        covs = [r[f"top{k}_coverage"] for r in results]
        avg = np.mean(covs)
        lo = lags_only.get(k, 0)
        baseline = baselines.get(k, 0)
        delta = avg - baseline
        delta_str = f"+{delta:.1%}" if delta > 0 else f"{delta:.1%}"
        print(f"| Top-{k} | {avg:.1%} | {lo:.1%} | {baseline:.1%} | {delta_str} |")
    ...
    for pos in range(1, 5):
        ...
        print(f"QS{pos}: Top-1={t1:.1%}, Top-3={t3:.1%}, Rank={rank:.2f}")
    ...
    # Rolling feature impact
    roll_imp = sum(imp for f, imp in feat_imp if "roll_" in f)
    print(f"\nRolling mean features importance: {roll_imp:.4f}")

    output = {
        "experiment": "lags_plus_rolling_means",
        "lags": list(args.lags),
    ...
    out_path = os.path.join(args.out_dir, "holdout_lags_rolling_results.json")
